...
def test(model, output_key, plot):
    ...
    if plot:
        ...
        plot = Plotting(
            "kdv_diff_lbfgs_right_" + output_key,
            data.lb_sol,
            data.ub_sol,
            data.X_sol_star,
...
class LossBoundary(MSELoss):
    ...
    def forward(self, output_dict, label_dict, weight_dict=None):
        ...
        out_dict = {"ub": u_lb, "du_x_b": du_x_lb, "du_xx_b": du_xx_lb}
        label_dict = {"ub": u_ub, "du_x_b": du_x_ub, "du_xx_b": du_xx_ub}
        # losses = super().forward(out_dict, label_dict, weight_dict)
...
if __name__ == "__main__":
    ...
    ITERS_PER_EPOCH = 1
    LEARNING_RATE = 1e-3
    DATASET_PATH = "../dataset_gen/KdV_sine.mat"
    DATASET_PATH_SOL = "../dataset_gen/KdV_cos.mat"
    OUTPUT_DIR = "../outs_kdv_diff_lbfgs/"
    ...
    t_ub = paddle.to_tensor([40.0], dtype="float32")
    x_lb = paddle.to_tensor([-20.0], dtype="float32")
    x_ub = paddle.to_tensor([20.0], dtype="float32")

    # initialize models
    model_idn = ppsci.arch.MLP(("t", "x"), ("u_idn",), 4, 50, "sin")
    model_pde = ppsci.arch.MLP(("u_x", "du_x", "du_xx", "du_xxx"), ("f_pde",), 2, 100, "sin", 4)
    ...
    solver = ppsci.solver.Solver(
        ...
        # checkpoint_path = OUTPUT_DIR+"checkpoints/test"
    )
    ...
    train_dataloader_cfg_sol_bc = {
        "dataset": {
            ...
            "file_path": DATASET_PATH_SOL,
            "input_keys": ("t", "x"),
            "label_keys": ("ub_sol", "du_x_sol", "du_xx_sol"),
            "alias_dict": {"t": "tb", "x": "xb", "ub_sol": "xb", "du_x_sol": "xb", "du_xx_sol": "xb"},
        },
    }
