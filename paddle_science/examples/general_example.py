...
class LossBoundary(MSELoss):
    ...
    def __call__(self, output_dict, label_dict, weight_dict=None):
        ...
        label_dict = {}
        losses = 0
        for key in sol_sup_bc_dict.keys():
            d_u_x = output_dict[key]
            u_x_lb, u_x_ub = paddle.split(d_u_x, 2, axis=0)
            losses += super().forward({"bc_loss": u_x_lb}, {"bc_loss": u_x_ub}, weight_dict)
        return losses
...
if __name__ == "__main__":
    ...
    EPOCHS = 50000
    MAX_ITER = 10000    # for LBFGS
    ITERS_PER_EPOCH = 1
    LEARNING_RATE = 1e-3
    OUTPUT_DIR = "../outs_ks_adam/"
    ...
    # initialize example's special parameters
    ...
    pde_input_keys = ("u_x", "du_x", "du_xx", "du_xxx", "du_xxxx")
    sol_sup_bc_dict = {
        "ub_sol": lambda out: out["u_sol"],
        "du_x_sol": lambda out: jacobian(out["u_sol"], out["x"]),
        "du_xx_sol": lambda out: hessian(out["u_sol"], out["x"]),
        "du_xxx_sol": lambda out: jacobian(hessian(out["u_sol"], out["x"]), out["x"]),
    }

    # initialize models
    model_idn = ppsci.arch.MLP(("t", "x"), ("u_idn",), 4, 50, "sin")
    model_pde = ppsci.arch.MLP(pde_input_keys, ("f_pde",), 2, 100, "sin", len(pde_input_keys))
    ...
    def transform_f(input,model_trans,out_key):
        ...
        input_trans = {"u_x": u}
        last_jacobian = u
        for i in range(1, len(pde_input_keys)):
            new_jacobian = jacobian(last_jacobian, x)
            input_trans[pde_input_keys[i]] = new_jacobian
            last_jacobian = new_jacobian
        return input_trans
    ...
    def transform_f_sol(input):
        return transform_f(input,model_sol, "u_sol")
