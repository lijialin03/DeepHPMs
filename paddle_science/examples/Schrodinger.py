...
def test(model, output_key, plot, plot_uv=False):
    ...
    if mode in ["u_idn", "v_idn", "f_pde", "g_pde"]:
        ...
        output_key = mode.split('_')[0] + "_idn"
    ...
    if plot_uv:
        plot.figname = "nls_reuse_lbfgs_uv"
        pred_u = model_idn_u(input_test)["u_idn"].numpy()
        pred_v = model_idn_v(input_test)["v_idn"].numpy()
...
class LossBoundary(MSELoss):
    ...
    def forward(self, output_dict, label_dict, weight_dict=None):
        u_b, v_b = output_dict["ub_idn"], output_dict["vb_idn"]
...
if __name__ == "__main__":
    ...
    model_pde_f.register_input_transform(transform=transform_fg)
    model_pde_g.register_input_transform(transform=transform_fg)

    # initialize model list
    model_list = ppsci.arch.ModelList((model_idn_u, model_idn_v, model_pde_f, model_pde_g))
    ...
    # initialize a new opt with a small initial learning rate for lbfgs in case that loss explosion 
    optimizer_idn = ppsci.optimizer.LBFGS(learning_rate = 0.01, max_iter=MAX_ITER)([model_idn_u, model_idn_v])
    # maunally build constraint(s)
    ...
    sup_constraint_sol_bc = ppsci.constraint.SupervisedConstraint(
        ...
        LossBoundary("sum"),
        {
            "ub_idn": lambda out: out["u_idn"],
            "vb_idn": lambda out: out["v_idn"],
    ...
    test(model_idn_u, "u_sol", True)
    test(model_idn_v, "v_sol", True, True)
