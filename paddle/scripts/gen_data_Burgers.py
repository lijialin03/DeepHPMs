...
def func(u_im1, u_i, u_ia1):
    u_na1 = (
        u_i
        - c * (dt / dx) * u_i * (u_i - u_im1)
        + d * (u_ia1 - 2 * u_i + u_im1) * (dt / (dx * dx))
    )
    return u_na1
...
u_tx = np.zeros((nx + 1, nt + 801))
u_tx[:, 0] = u0
for n in range(1, nt + 801):
    u_tx[:, n] = u_tx[:, n - 1]
    u_tx[-1][n - 1] = u_tx[0][n - 1]
    for i in range(1, nx):
        u_tx[i][n] = func(u_tx[i - 1][n - 1], u_tx[i][n - 1], u_tx[i + 1][n - 1])
    u_tx[0][n] = func(u_tx[-2][n - 1], u_tx[0][n - 1], u_tx[1][n - 1])
    # u_tx[0][n] = 0.0

plt.plot(x_all[:-1], u_tx[:-1, -1], label="converged")
plt.plot(x_all[:-1], Exact_idn[:, -1], label="Exact_idn")
plt.text(6, 0.9, "c = 0.15", fontdict={"family": "serif", "size": 10, "color": "red"})
plt.text(6, 0.8, "d = 0.03", fontdict={"family": "serif", "size": 10, "color": "red"})
...
plot = Plotting("test_gen", [0.0, -8.0], [50.0, 8.0], X_idn_star, (T_idn, X_idn))
