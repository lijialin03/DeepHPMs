...
class DeepHPM:
    def __init__(
        ...
        # Init for Solution
        self.sol_init(t_b, x_b, y_b, w_b, t_f, x_f, y_f, u_f, v_f, layers)
    ...
    def idn_init(self, t, x, y, u, v, w, w_layers, pde_layers):
        ...
        self.idn_f_pred = self.idn_net_f(
            self.t_tf, self.x_tf, self.y_tf, self.u_tf, self.v_tf
        )
        ...
        self.idn_w_optimizer = tf.contrib.opt.ScipyOptimizerInterface(
            self.idn_w_loss,
        ...
        self.idn_w_train_op_Adam = self.idn_w_optimizer_Adam.minimize(
            self.idn_w_loss, var_list=self.w_weights + self.w_biases
        )
    ...
    def sol_init(self, t_b, x_b, y_b, w_b, t_f, x_f, y_f, u_f, v_f, layers):
        ...
        # self.w_weights, self.b_biases = initialize_NN(layers)
        ...
        self.sol_f_pred = self.sol_net_f(
            self.t_f_tf, self.x_f_tf, self.y_f_tf, self.u_f_tf, self.v_f_tf
        )

        # loss for Solution
        self.sol_loss = tf.reduce_sum(
        ...
        self.sol_train_op_Adam = self.sol_optimizer_Adam.minimize(
            self.sol_loss, var_list=self.w_weights + self.w_biases
        )
    ...
    def sol_predict(self, t_star, x_star, y_star):
        u_star = self.sess.run(
            self.w_b_pred,
            {self.t_b_tf: t_star, self.x_b_tf: x_star, self.y_b_tf: y_star},
        )
...
if __name__ == "__main__":
    ...
    error_w_identifier = np.linalg.norm(w_star - w_pred_identifier, 2) / np.linalg.norm(
        w_star, 2
    )
    ...
    W_data_plot = griddata(
        X_data, w_data[:, snap].flatten(), (X_plot, Y_plot), method="cubic"
    )
    W_pred_plot = griddata(
        X_data, w_pred[:, snap].flatten(), (X_plot, Y_plot), method="cubic"
    )
    ...
    savefig("../fig_test/NavierStokes_normal", crop=False)
