...
class DeepHPM:
    ...
    def idn_init(self, t, x, u, u_layers, pde_layers):
        ...
        self.idn_f_pred, self.ut_pde = self.idn_net_f(self.t_tf, self.x_tf)
        ...
        self.idn_u_optimizer_Adam = tf.train.AdamOptimizer(learning_rate=0.0001)
        self.idn_u_train_op_Adam = self.idn_u_optimizer_Adam.minimize(
            self.idn_u_loss, var_list=self.u_weights + self.u_biases
        )

        self.idn_f_optimizer_Adam = tf.train.AdamOptimizer(learning_rate=0.0001)
    ...
    def idn_f_train(self, N_iter):
        ...
        for it in range(N_iter):
            ...
            if it % 1000 == 0:
                elapsed = time.time() - start_time
                loss_value = self.sess.run([self.idn_f_loss, self.ut_pde], tf_dict)
                # print("u_t: ", loss_value[-1], end=", ")
                print("It: %d, Loss: %.3e, Time: %.2f" % (it, loss_value[0], elapsed))
                start_time = time.time()

        self.idn_f_optimizer.minimize(
            self.sess,
            feed_dict=tf_dict,
    ...
    def sol_init(self, x0, u0, tb, X_f, layers):
        ...
        self.sol_f_pred, self.ut = self.sol_net_f(self.t_f_tf, self.x_f_tf)
        idn_f_pred, _ = self.idn_net_f(self.t_f_tf, self.x_f_tf)

        # loss for Solution
        self.loss0 = tf.reduce_mean(tf.square(self.u0_tf - self.u0_pred))
        self.loss1 = tf.reduce_mean(tf.square(self.sol_f_pred))
        self.loss2 = tf.reduce_mean(tf.square(self.u_lb_pred - self.u_ub_pred))
        self.loss3 = tf.reduce_mean(tf.square(self.u_x_lb_pred - self.u_x_ub_pred))
        self.loss_test = tf.reduce_mean(tf.square(idn_f_pred))

        self.sol_loss = self.loss0 + self.loss1 + self.loss2 + self.loss3
        # self.sol_loss = self.loss1 + self.loss2 + self.loss3
        # self.sol_loss = (
        ...
        self.sol_optimizer_Adam = tf.train.AdamOptimizer(learning_rate=0.001)
    ...
    def sol_train(self, N_iter):
        ...
        for it in range(N_iter):
            ...
            if it % 1000 == 0:
                ...
                # print("It: %d, Loss: %.3e, Time: %.2f" % (it, loss_value, elapsed))
                print(
                    "It: %d, Time: %.2f, Loss: %.3e" % (it, elapsed, loss_value[0]),
                    end="",
                )
                for i in range(1, len(loss_value) - 1):
                    print(", Loss%d: %.3e" % (i - 1, loss_value[i]), end="")
                # print(", u_t: ", loss_value[-1], end="")
                print("")
                start_time = time.time()

        self.sol_optimizer.minimize(
            self.sess,
            feed_dict=tf_dict,
...
if __name__ == "__main__":
    ...
    error_u_identifier = np.linalg.norm(
        u_idn_star - u_pred_identifier, 2
    ) / np.linalg.norm(u_idn_star, 2)
    ...
    error_u_idn = np.linalg.norm(u_idn_star - u_pred_idn, 2) / np.linalg.norm(
        u_idn_star, 2
    )
    ...
    # ax.axis("off")
    fig = plt.figure("Burgers", figsize=(10, 6))
