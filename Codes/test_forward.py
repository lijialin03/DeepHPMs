...
class DeepHPM:
    ...
    def show_pred(self, it, pred, mark):
        pred = np.squeeze(pred)
        print("It: %d, %s[:10]:" % (it, mark))
        print(pred[:10])
    ...
    def idn_u_train(self, N_iter):
        ...
        for it in range(N_iter):
            ...
            if it % 1000 == 0 or it >= N_iter - 10:
                u_pred = self.sess.run([self.idn_u_pred], tf_dict)
                self.show_pred(it, u_pred, "u_pred")
    ...
    def idn_f_train(self, N_iter):
        ...
        for it in range(N_iter):
            ...
            if it % 1000 == 0 or it >= N_iter - 10:
                f_pred = self.sess.run([self.idn_f_pred], tf_dict)
                self.show_pred(it, f_pred, "f_pred")
    ...
    def sol_train(self, N_iter):
        ...
        for it in range(N_iter):
            ...
            if it % 1000 == 0 or it >= N_iter - 10:
                u_pred, f_pred = self.sess.run([self.u0_pred, self.sol_f_pred], tf_dict)
                self.show_pred(it, u_pred, "u_pred")
                self.show_pred(it, f_pred, "f_pred")
...
if __name__ == "__main__":
    ...
    with open("../X_f_train_forward.txt", "w") as f:
