...
class DeepHPM:
    ...
    def idn_init(self, t, x, u, v, uv_layers, pde_layers):
        ...
        self.idn_uv_loss = tf.reduce_sum(
            tf.square(self.idn_u_pred - self.u_tf)
        ) + tf.reduce_sum(tf.square(self.idn_v_pred - self.v_tf))
        self.idn_fg_loss = tf.reduce_sum(tf.square(self.idn_f_pred)) + tf.reduce_sum(
            tf.square(self.idn_g_pred)
        )

        # Optimizer for Identification
        self.idn_uv_optimizer = tf.contrib.opt.ScipyOptimizerInterface(
            self.idn_uv_loss,
        ...
        self.idn_fg_optimizer = tf.contrib.opt.ScipyOptimizerInterface(
            self.idn_fg_loss,
            var_list=self.pde_u_weights
            + self.pde_u_biases
            + self.pde_v_weights
            + self.pde_v_biases,
            method="L-BFGS-B",
        ...
        self.idn_uv_train_op_Adam = self.idn_uv_optimizer_Adam.minimize(
            self.idn_uv_loss,
            var_list=self.u_weights + self.u_biases + self.v_weights + self.v_biases,
        )

        self.idn_fg_optimizer_Adam = tf.train.AdamOptimizer()
        self.idn_fg_train_op_Adam = self.idn_fg_optimizer_Adam.minimize(
            self.idn_fg_loss,
            var_list=self.pde_u_weights
            + self.pde_u_biases
            + self.pde_v_weights
    ...
    def idn_uv_train(self, N_iter):
        ...
        self.idn_uv_optimizer.minimize(
            self.sess,
            feed_dict=tf_dict,
    ...
    def idn_fg_train(self, N_iter):
        ...
        self.idn_fg_optimizer.minimize(
            self.sess,
            feed_dict=tf_dict,
    ...
    def sol_init(self, x0, u0, v0, tb, X_f, layers):
        ...
        self.u_weights, self.u_biases = initialize_NN(layers)
        self.v_weights, self.v_biases = initialize_NN(layers)
        ...
        (
            ...
            self.u_x_lb_pred,
            self.v_x_lb_pred,
        ) = self.sol_net_uv(self.t_lb_tf, self.x_lb_tf)
        (
            self.u_ub_pred,
            self.v_ub_pred,
            self.u_x_ub_pred,
            self.v_x_ub_pred,
        ) = self.sol_net_uv(self.t_ub_tf, self.x_ub_tf)
    ...
    def sol_predict(self, t_star, x_star):
        ...
        g_star = self.sess.run(
            self.sol_g_pred, {self.t_f_tf: t_star, self.x_f_tf: x_star}
        )
...
if __name__ == "__main__":
    ...
    (
        ...
        f_pred_identifier,
        g_pred_identifier,
    ) = model.idn_predict(t_idn_star, x_idn_star)
    ...
    error_v_identifier = np.linalg.norm(
        v_idn_star - v_pred_identifier, 2
    ) / np.linalg.norm(v_idn_star, 2)
    ...
    u_pred_idn, v_pred_idn, f_pred_idn, g_pred_idn = model.sol_predict(
        t_idn_star, x_idn_star
    )
    ...
    error_v_idn = np.linalg.norm(v_idn_star - v_pred_idn, 2) / np.linalg.norm(
        v_idn_star, 2
    )
